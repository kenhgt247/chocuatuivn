...
const App: React.FC = () => {
  ...
  // Dùng ref để lưu số dư cũ nhằm so sánh
  const prevBalanceRef = useRef<number>(0);

  // Khởi tạo: Kiểm tra user login & Thiết lập Realtime Listener
  useEffect(() => {
    let unsubscribe: () => void;

    const initialize = async () => {
      try {
        // 1. Lấy user hiện tại (Auth)
        ...
        if (currentUser) {
            ...
            // 2. [QUAN TRỌNG] Lắng nghe thay đổi User (Số dư, Trạng thái) Realtime
            // Hàm này cần được hỗ trợ trong db.ts (dùng onSnapshot của Firestore)
            if (db.onUserChange) {
                unsubscribe = db.onUserChange(currentUser.id, (updatedUser) => {
                    setUser(updatedUser);

                    // Logic: Nếu số dư mới > số dư cũ -> Báo "Tiền về"
                    if (updatedUser.walletBalance > prevBalanceRef.current) {
                        const amount = updatedUser.walletBalance - prevBalanceRef.current;
                        // Bạn có thể thay alert bằng thư viện Toast (như react-toastify) cho đẹp
                        alert(`💰 Ting Ting! Ví của bạn vừa được cộng ${formatPrice(amount)}`);
                    }
                    
                    // Cập nhật lại số dư cũ
  ...
  const handleLogin = (u: User) => {
      setUser(u);
      prevBalanceRef.current = u.walletBalance; // Reset mốc so sánh
  };
