...
// Import thêm để xử lý thông báo đẩy (Cần cấu hình Firebase của bạn)
...
const Layout: React.FC<LayoutProps> = ({ children, user }) => {
  ...
  useEffect(() => {
    if (user?.id) {
      const unsubChats = db.getChatRooms(user.id, (rooms) => setChatRooms(rooms));
      return () => unsubChats();
    } else {
      setChatRooms([]);
    }
  ...
  // --- 3. [NÂNG CẤP] XIN QUYỀN + LẤY TOKEN ĐỂ BẮN TIN KHI TẮT APP ---
  const handleEnableNotifications = async () => {
    ...
    try {
      // A. Xin quyền hiển thị thông báo
      const permission = await Notification.requestPermission();
      setNotifPermission(permission);

      if (permission === 'granted') {
        // B. Nếu đồng ý -> Cập nhật Badge ngay lập tức
        if ('setAppBadge' in navigator) navigator.setAppBadge(unreadChatCount);

        // C. [QUAN TRỌNG] Lấy Token để Server bắn tin khi tắt App
        // Lưu ý: Bạn cần file firebase config đã khởi tạo app
        try {
            const messaging = getMessaging(); 
            const currentToken = await getToken(messaging, { 
              // Thay bằng VAPID Key của bạn từ Firebase Console -> Project Settings -> Cloud Messaging
            ...
            if (currentToken) {
              ...
              if (user?.id) {
                  // Gửi token này lên server lưu vào thông tin user
                  // Ví dụ: await db.saveUserDeviceToken(user.id, currentToken);
                  console.log("Đã lưu token thiết bị để nhận thông báo nền!");
              }
              alert("✅ Đã bật thông báo thành công! Bạn sẽ nhận được tin nhắn kể cả khi thoát ứng dụng.");
            } else {
              console.log('Không lấy được Token.');
            }
        } catch (err) {
            console.log('Lỗi lấy Token FCM (Có thể do chưa cấu hình VAPID Key):', err);
            // Vẫn báo thành công phần quyền cơ bản
            alert("✅ Đã cấp quyền thông báo trên thiết bị!");
        }
      }
    } catch (error) {
  ...
  // --- 4. CẬP NHẬT SỐ ĐỎ TRÊN ICON (KHI ĐANG MỞ APP) ---
  useEffect(() => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch (err) { alert("Lỗi ảnh. Thử lại sau."); }
        ...
        <div className="flex items-center gap-1 md:gap-4 flex-shrink-0">
          ...
          {user && notifPermission === 'default' && (
            <button 
              ...
              className="flex items-center gap-1 bg-red-500 text-white px-3 py-1.5 rounded-full text-[10px] font-black border border-red-400 animate-pulse shadow-lg md:hidden"
            >
              🔔 Bật thông báo
            </button>
          )}
