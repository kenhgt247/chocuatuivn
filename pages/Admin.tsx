                    ...
                    <div className="space-y-6 pt-6 border-t border-gray-100">
                        <div className="flex items-center justify-between">
                             ...
                             <button type="button" onClick={() => {
                                 ...
                                 }} className="bg-primary/10 text-primary px-4 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-primary/20 transition-colors">+ Thêm Slide</button>
                        </div>
                        <div className="space-y-6">
                                   ...
                                   <div className="flex justify-between items-center pb-4 border-b border-gray-100">
                                       <div className="flex items-center gap-3">
                                           ...
                                           <select value={slide.type || 'text'} onChange={e => {
                                               ...
                                               <option value="text">Dạng Chữ</option><option value="image">Dạng Ảnh</option>
                                           </select>
                                       </div>
                                       <div className="flex items-center gap-3">
                                           <label className="flex items-center cursor-pointer gap-2"><span className="text-[9px] font-black text-gray-400 uppercase">{slide.isActive ? 'Hiển thị' : 'Ẩn'}</span><input type="checkbox" className="hidden" checked={slide.isActive} onChange={e => {
                                       ...
                                       <div className="flex gap-4 items-start">
                                           <div className="w-1/3 aspect-[3/1] bg-gray-100 rounded-2xl overflow-hidden border border-gray-200 relative group">
                                               ...
                                               <label className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer transition-opacity text-white text-[10px] font-black">TẢI ẢNH<input type="file" className="hidden" accept="image/*" onChange={async (e) => {
                                                   if(e.target.files?.[0]) { setIsLoading(true); try { const compressed = await compressAndGetBase64(e.target.files[0]); const url = await db.uploadImage(compressed, `banners/${Date.now()}.jpg`); const ns = [...settings.bannerSlides]; ns[idx].imageUrl = url; setSettings({...settings, bannerSlides: ns}); } catch (err) { alert("Lỗi tải ảnh"); } setIsLoading(false); }
                                               }} /></label>
                                           </div>
