...
const Admin: React.FC<{ user: User | null }> = ({ user }) => {
  ...
  // [MỚI] Hàm xử lý Nâng cấp dữ liệu cũ
  const handleFixOldData = async () => {
    if (!window.confirm("CẢNH BÁO: Thao tác này sẽ quét toàn bộ database để thêm trường Keywords, Slug, ViewCount cho các tin cũ chưa có. Việc này có thể mất thời gian nếu dữ liệu lớn. Bạn có chắc chắn?")) return;
    ...
    if (db.fixOldData) {
        ...
        alert("Hàm fixOldData chưa được định nghĩa trong db.ts");
    }
    setIsLoading(false);
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 pb-24 relative min-h-screen">
      ...
      <div className="flex-1 min-w-0 space-y-6">
         ...
         {/* SETTINGS (ĐÃ SỬA ĐỂ HIỆN NÚT FIX DATA) */}
         {activeTab === 'settings' && settings && (
             <div className="bg-white border border-borderMain rounded-[2.5rem] p-8 shadow-soft">
                 <form onSubmit={handleSaveSettings} className="space-y-12">
                    ...
                    {/* Các phần cấu hình khác (Phí, Banner, Tier, Bank) giữ nguyên như cũ... */}
                    <div className="space-y-6">
                        <h4 className="text-sm font-black uppercase tracking-widest text-primary flex items-center gap-2"><span className="w-2 h-2 bg-primary rounded-full"></span> Quản lý Chiết khấu & Giá</h4>
                        <div className="grid md:grid-cols-3 gap-6 bg-gray-50 p-6 rounded-[2rem]"><div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase ml-2">Giá Đẩy Tin (đ)</label><input type="number" value={settings.pushPrice} onChange={e => setSettings({...settings, pushPrice: parseInt(e.target.value)})} className="w-full bg-white border border-gray-200 rounded-xl p-3 font-bold" /></div><div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase ml-2">Giảm giá Đẩy Tin (%)</label><input type="number" value={settings.pushDiscount} onChange={e => setSettings({...settings, pushDiscount: parseInt(e.target.value)})} className="w-full bg-white border border-gray-200 rounded-xl p-3 font-bold text-red-500" /></div><div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase ml-2">Giảm giá Gói VIP (%)</label><input type="number" value={settings.tierDiscount} onChange={e => setSettings({...settings, tierDiscount: parseInt(e.target.value)})} className="w-full bg-white border border-gray-200 rounded-xl p-3 font-bold text-red-500" /></div></div>
                    </div>
                    ...
                    <div className="space-y-6 pt-6 border-t border-gray-100">
                        ...
                        <div className="grid md:grid-cols-3 gap-6">
                            ...
                            <div className="bg-yellow-50 p-6 rounded-3xl border border-yellow-100">
                                <h5 className="font-black text-sm text-yellow-700">Nâng cấp Dữ liệu cũ</h5>
                                <p className="text-[11px] text-yellow-600/70 mb-4 font-medium">Thêm trường Keywords, Slug cho các tin đăng phiên bản cũ.</p>
                                <button type="button" onClick={handleFixOldData} className="bg-yellow-500 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-yellow-200 hover:scale-105 transition-transform w-full">Chạy nâng cấp</button>
                            </div>
                            ...
                            <div className="bg-red-50 p-6 rounded-3xl border border-red-100">
                                ...
                                <button type="button" onClick={async () => {if(window.confirm("CẢNH BÁO: Hành động này sẽ xóa dữ liệu cũ và tạo mới dữ liệu ảo. Tiếp tục?")){setIsLoading(true); await db.seedDatabase(); setIsLoading(false); loadInitialData();}}} className="bg-red-500 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-red-200 hover:scale-105 transition-transform w-full">Bắt đầu Seed</button>
                            </div>

                            <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100">
                                <h5 className="font-black text-sm text-blue-700">Sitemap SEO</h5>
                                <p className="text-[11px] text-blue-600/70 mb-4 font-medium">Tạo file sitemap.xml chứa toàn bộ link sản phẩm.</p>
                                <button type="button" onClick={handleDownloadSitemap} className="bg-blue-600 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase shadow-lg shadow-blue-200 hover:scale-105 transition-transform w-full">Tải Sitemap.xml</button>
                            </div>
                        </div>
                    </div>
