  ...
  return (
    <div className="max-w-7xl mx-auto md:px-4 lg:px-8 py-0 md:py-8 space-y-6 pb-32 md:pb-24">
      ...
      <nav className="flex items-center gap-2 text-[10px] md:text-xs font-black uppercase tracking-widest text-gray-400 px-4 md:px-0 overflow-x-auto no-scrollbar whitespace-nowrap pt-4 md:pt-0">
        <Link to="/" className="hover:text-primary transition-colors flex-shrink-0">Chợ Của Tui</Link>
        <span>/</span>
        {currentCategory ? (
          <Link to={`/danh-muc/${currentCategory.slug}`} className="hover:text-primary transition-colors flex-shrink-0">{currentCategory.name}</Link>
      ...
      <div className="grid lg:grid-cols-12 gap-0 md:gap-8 bg-white md:bg-transparent overflow-hidden">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <div className="relative bg-black aspect-square md:aspect-video md:rounded-3xl overflow-hidden group">
            ...
            {listing.images.length > 1 && (
              <>
                <button onClick={() => setActiveImage(prev => prev > 0 ? prev - 1 : listing.images.length - 1)} className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-white/90 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity z-10">
                ...
                <button onClick={() => setActiveImage(prev => prev < listing.images.length - 1 ? prev + 1 : 0)} className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-white/90 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity z-10">
          ...
          {listing.images.length > 1 && (
            <div className="hidden md:flex gap-3 overflow-x-auto no-scrollbar py-2">
              {listing.images.map((img, idx) => (
                <button key={idx} onClick={() => setActiveImage(idx)} className={`w-24 h-24 rounded-2xl overflow-hidden border-4 transition-all flex-shrink-0 ${activeImage === idx ? 'border-primary' : 'border-transparent opacity-50 hover:opacity-100'}`}>
          ...
          <div className="bg-white md:rounded-[2.5rem] p-6 md:p-8 border-t md:border border-gray-100 shadow-sm md:shadow-soft space-y-6">
            <div className="space-y-4">
               {/* --- MOBILE INFO BLOCK (HIỆN TRÊN MOBILE ĐỂ CÂN BẰNG) --- */}
               <div className="md:hidden space-y-4 mb-6 border-b border-gray-100 pb-6">
                  <div className="space-y-2">
                    <h1 className="text-lg font-bold text-gray-900 leading-tight">{listing.title}</h1>
                    <p className="text-2xl font-black text-red-600">{formatPrice(listing.price)}</p>
                  ...
                  {/* Seller Card Mobile (Đưa lên đây để người dùng Mobile thấy ngay) */}
                  <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-2xl border border-gray-100">
                    <Link to={`/seller/${listing.sellerId}`} className="shrink-0">
                        <img src={listing.sellerAvatar || "https://via.placeholder.com/50"} alt={listing.sellerName} className="w-10 h-10 rounded-full object-cover" />
                    </Link>
                    <div className="min-w-0 flex-1">
                        <Link to={`/seller/${listing.sellerId}`} className="font-bold text-sm text-gray-900 truncate block">{listing.sellerName}</Link>
                        <div className="flex items-center gap-2 text-[10px] text-gray-500">
                            <span className="text-green-600 font-bold">● Online</span>
                            <span>•</span>
                            <span>{(seller as any)?.rating || 5.0} ★</span>
                        </div>
                    </div>
                    <Link to={`/seller/${listing.sellerId}`} className="text-[10px] font-bold border border-primary text-primary px-3 py-1.5 rounded-full">Xem trang</Link>
                  </div>
               </div>
               {/* -------------------------------------------------------- */}
            ...
            <div className="pt-6 border-t border-gray-100">
               <div className="flex flex-wrap gap-4">
                 ...
                 <div className="bg-bgMain px-4 py-2 rounded-xl text-xs font-bold text-gray-500">Danh mục: <span className="text-textMain">{currentCategory?.name || listing.category}</span></div>
               </div>
            </div>
          </div>
        ...
        {/* --- CỘT PHẢI (4 phần) - CHỈ HIỆN TRÊN DESKTOP (Hidden Mobile) --- */}
        <div className="hidden md:block lg:col-span-4 p-4 md:p-0">
          <div className="bg-white md:rounded-[2.5rem] p-6 md:p-8 md:border border-gray-100 md:shadow-soft space-y-6 sticky top-24">
            <div className="space-y-2">
              <p className="text-3xl font-black text-red-600">{formatPrice(listing.price)}</p>
            ...
            <div className="pt-6 border-t border-gray-100 space-y-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                <button onClick={handleStartChat} className="w-full bg-primary text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-primary/20 hover:bg-primaryHover active:scale-95 transition-all flex items-center justify-center gap-2">
                ...
                {seller?.phone && (
                  isPhoneVisible ? (
                    <a href={`tel:${seller.phone}`} className="w-full bg-white border-2 border-primary text-primary py-4 rounded-2xl font-black text-xs uppercase tracking-widest text-center hover:bg-primary/5 active:scale-95 transition-all flex items-center justify-center gap-2">
                    ...
                    <button onClick={() => setIsPhoneVisible(true)} className="w-full bg-white border-2 border-primary text-primary py-4 rounded-2xl font-black text-xs uppercase tracking-widest text-center hover:bg-primary/5 active:scale-95 transition-all flex items-center justify-center gap-2">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" strokeWidth={2}/></svg>
              ...
              <div className="flex gap-3">
                <button onClick={handleToggleFav} className="flex-1 flex items-center justify-center gap-2 py-3.5 border-2 border-gray-100 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-50 transition-all">
                ...
                <button onClick={() => setIsShareModalOpen(true)} className="flex-1 flex items-center justify-center gap-2 py-3.5 border-2 border-gray-100 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-gray-50 transition-all">
              ...
              <div className="pt-4">
                <button onClick={() => setShowReportModal(true)} className="w-full text-[10px] font-black text-gray-400 uppercase tracking-widest hover:text-red-500 transition-colors py-2 flex items-center justify-center gap-2">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 z-[100] md:hidden flex gap-2 pb-6 shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
        <button onClick={handleStartChat} className="flex-1 bg-white border-2 border-primary text-primary py-3 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-primary/5 active:scale-95 transition-all flex items-center justify-center gap-2">
           <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" strokeWidth={2}/></svg>
           Chat
        </button>
        {seller?.phone && (
           <a href={`tel:${seller.phone}`} className="flex-1 bg-primary text-white py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-primary/30 active:scale-95 transition-all flex items-center justify-center gap-2">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" strokeWidth={2}/></svg>
              Gọi ngay
           </a>
        )}
      ...
      {showReportModal && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative animate-fade-in-up border border-borderMain">
            ...
            <div className="space-y-4">
              <select value={reportReason} onChange={(e) => setReportReason(e.target.value)} className="w-full bg-bgMain border border-borderMain rounded-2xl p-4 font-bold text-sm">
                  <option value="">Chọn lý do...</option>
                  {REPORT_REASONS.map(r => <option key={r} value={r}>{r}</option>)}
              </select>
              <textarea rows={3} placeholder="Mô tả thêm..." value={reportDetails} onChange={(e) => setReportDetails(e.target.value)} className="w-full bg-bgMain border border-borderMain rounded-2xl p-4 text-sm" />
              <div className="flex gap-3 pt-4">
                ...
                <button onClick={handleReport} className="flex-1 py-4 rounded-2xl font-black text-[11px] uppercase bg-red-500 text-white">Gửi báo cáo</button>
              </div>
            </div>
          </div>
        </div>
