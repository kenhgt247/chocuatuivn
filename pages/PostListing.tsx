...
import { analyzeListingImages } from '../services/geminiService'; // Vẫn giữ AI cho ảnh thường
import { getLocationFromCoords } from '../utils/locationHelper';
import { compressAndGetBase64 } from '../utils/imageCompression';
import { crawlLinkMetadata } from '../utils/crawler'; // Import Crawler Client-side cũ
...
const PostListing: React.FC<{ user: User | null }> = ({ user }) => {
  ...
  // --- 2. XỬ LÝ LẤY TIN (CRAWLER CLIENT CŨ) ---
  const handleFetchLink = async () => {
      ...
      try {
          // Gọi hàm Crawler ở phía Client (src/utils/crawler.ts)
          const result = await crawlLinkMetadata(affiliateLinkInput);
          ...
          if (result.success && result.data) {
             ...
             alert(`✅ Đã lấy tin thành công từ ${result.data.brand}!`);
          } else {
             throw new Error(result.error || "Không lấy được thông tin");
          }
      } catch (e) {
          console.error(e);
          setManualAffiliateMode(true);
          setFormData(prev => ({ ...prev, affiliateLink: affiliateLinkInput }));
          alert("⚠️ Link này bảo mật cao không lấy tự động được.\n\n👉 Vui lòng TẢI ẢNH LÊN và nhập THỦ CÔNG nhé!");
      } finally {
          setIsCrawling(false);
      }
  };

  // --- 3. CÁC TRƯỜNG CHI TIẾT (BẠN SỢ MẤT CÁI NÀY ĐÚNG KHÔNG? NÓ ĐÂY!) ---
  ...
  // --- 4. UPLOAD ẢNH & AI (VẪN GIỮ NGUYÊN) ---
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        // Chỉ chạy AI cho tin thường (để điền thông tin tự động cho người dùng đỡ mỏi tay)
        if (compressedResults.length > 0 && listingType === 'normal') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (listingType === 'affiliate' && !formData.affiliateLink) {
        return alert('Thiếu Link tiếp thị liên kết.');
    }
    ...
    try {
      ...
      // Upload ảnh nếu là base64 (ảnh tải từ máy)
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 px-4 pb-20 pt-6 font-sans">
      ...
      <div className="bg-gray-100 p-1 rounded-xl flex max-w-md mx-auto">
          <button onClick={() => { setListingType('normal'); setFormData(p => ({...p, title: '', price: '', images: [], affiliateLink: null})); }} className={`flex-1 py-3 rounded-lg text-xs font-bold uppercase ${listingType === 'normal' ? 'bg-white shadow text-primary' : 'text-gray-400'}`}>📦 Bán đồ cũ</button>
          <button onClick={() => { setListingType('affiliate'); setFormData(p => ({...p, title: '', price: '', images: []})); }} className={`flex-1 py-3 rounded-lg text-xs font-bold uppercase ${listingType === 'affiliate' ? 'bg-gradient-to-r from-orange-500 to-red-500 text-white shadow' : 'text-gray-400'}`}>💰 Tiếp thị liên kết</button>
      </div>
           ...
           {listingType === 'affiliate' && !isVip ? (
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
                ...
                {listingType === 'affiliate' && (
                    <div className="space-y-2 mb-4">
                        <div className="flex gap-2">
                            <input type="url" value={affiliateLinkInput} onChange={e => setAffiliateLinkInput(e.target.value)} placeholder="Dán link Shopee/Lazada..." className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs font-bold outline-none focus:border-orange-500"/>
                            <button type="button" onClick={handleFetchLink} disabled={isCrawling || !affiliateLinkInput} className="bg-orange-500 text-white px-3 rounded-lg text-[10px] font-black uppercase whitespace-nowrap">
                                {isCrawling ? '...' : 'Lấy tin'}
                            </button>
                        </div>
                        {manualAffiliateMode && (
                            <div className="bg-red-50 border border-red-100 p-2 rounded-lg text-center animate-fade-in">
                                <p className="text-[10px] text-red-600 font-bold italic">⚠️ Không lấy được tự động.</p>
                                <p className="text-[9px] text-red-500">Vui lòng tải ảnh lên và nhập tay.</p>
                            </div>
                        )}
                    </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  {/* NÚT TẢI ẢNH THỦ CÔNG (LUÔN HIỆN) */}
        ...
        <div className="lg:col-span-2">
          {(listingType === 'normal' || isVip) && (
             <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
                ...
                {/* GỌI HÀM RENDER CÁC TRƯỜNG CHI TIẾT */}
                {renderDynamicFields()}
