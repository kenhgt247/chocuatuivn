...
const PostListing: React.FC<{ user: User | null }> = ({ user }) => {
  ...
  const [priceSuggestions, setPriceSuggestions] = useState<{fast: number, market: number, high: number} | null>(null); // Giữ logic gợi ý giá nếu có
  ...
  const runAIAnalysis = async (images: string[]) => {
    ...
    try {
      ...
      if (analysis) {
        ...
        if (detectedCategory) {
          if (detectedCategory.parentId) {
            ...
            foundParentId = detectedCategory.id;
             ...
             // Logic tìm kiếm mềm dẻo hơn nếu ID không khớp 100%
             const softMatch = categories.find(c => c.id.includes(analysis.category) || analysis.category.includes(c.id));
             if (softMatch) {
                 if (softMatch.parentId) {
                    foundChildId = softMatch.id;
                    foundParentId = softMatch.parentId;
                    newAttributes = softMatch.attributes || [];
                 } else {
                    foundParentId = softMatch.id;
                 }
             }
        }
        ...
        // Lưu gợi ý giá để hiển thị (Nếu trong code geminiService có trả về)
  ...
  const renderDynamicFields = () => {
    ...
    return (
      // [FIX MOBILE] Đổi grid-cols-2 thành grid-cols-1 md:grid-cols-2
          ...
          {(listingType === 'normal' || user?.subscriptionTier === 'pro') && (
            <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-3xl p-8 shadow-xl shadow-gray-100/50 space-y-6">
              ...
              <div className={`p-6 rounded-2xl border transition-all ${formData.isAuction ? 'bg-purple-50 border-purple-100' : 'bg-white border-transparent'}`}>
                ...
                {/* [NEW] Gợi ý giá AI (Thêm vào đây cho đủ bộ) */}
              ...
              {/* [ĐÃ CHUYỂN XUỐNG ĐÂY] KHỐI QUY TẮC & MẸO BÁN NHANH */}
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 p-6 rounded-2xl relative overflow-hidden">
                <div className="absolute top-0 right-0 -mr-4 -mt-4 w-24 h-24 bg-blue-100 rounded-full opacity-50 blur-2xl"></div>
                <div className="relative z-10">
                  <h3 className="flex items-center gap-2 font-black text-xs md:text-sm uppercase text-blue-600 mb-3 tracking-wider">
