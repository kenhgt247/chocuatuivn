...
const PostListing: React.FC<{ user: User | null }> = ({ user }) => {
  ...
  // --- STATE HẠN MỨC (LOGIC QUAN TRỌNG) ---
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // [LOGIC CHẶN] Nếu quá hạn mức -> Set cờ chặn
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // [CHECK LẠI LẦN CUỐI] Bảo mật 2 lớp
    ...
    try {
      ...
      if (!isEditing && newCount >= maxPosts) {
  ...
  // --- HÀM RENDER ATTRIBUTES (ĐÃ KHÔI PHỤC) ---
  const renderDynamicFields = () => {
    if (currentAttributes.length === 0) return null;
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-blue-50/50 p-5 rounded-3xl border border-blue-100 mb-6 animate-fade-in-up">
  ...
  // [GIAO DIỆN CHẶN ĐẸP MẮT - ĐÃ KHÔI PHỤC]
      ...
      <div className="text-center space-y-3 mb-8">
        ...
        {!isEditing && (
          <div className="flex flex-col items-center gap-2">
            <div className={`inline-flex items-center gap-3 px-5 py-2 rounded-full border shadow-sm ${remainingPosts <= 1 ? 'bg-red-50 border-red-200' : 'bg-white border-slate-200'}`}>
              ...
              <span className={`text-xs font-black ${remainingPosts <= 1 ? 'text-red-500 animate-pulse' : 'text-primary'}`}>
                CÒN {remainingPosts}/{maxPosts} LƯỢT
              </span>
            </div>
          </div>
        )}
      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-[3rem] p-8 md:p-12 shadow-xl space-y-8">
            ...
            {/* [KHÔI PHỤC] CẢNH BÁO TIN CUỐI TRONG FORM (NHẤP NHÁY VÀNG) */}
            {!isEditing && remainingPosts === 1 && (
                <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-r-xl mb-4 animate-pulse">
                  <div className="flex items-center gap-3">
                    <span className="text-2xl">⚠️</span>
                    <div>
                      <h4 className="text-xs font-black text-yellow-700 uppercase">Lưu ý quan trọng</h4>
                      <p className="text-[11px] text-yellow-800">
                        Đây là tin đăng <strong>cuối cùng</strong> trong ngày. Sau tin này, bạn cần <Link to="/upgrade" className="font-bold underline ml-1">Nâng cấp VIP</Link> để đăng tiếp.
                      </p>
                    </div>
                  </div>
                </div>
            ...
            <button type="submit" disabled={loading} className={`w-full py-5 rounded-[2rem] font-black text-xs uppercase tracking-[0.2em] shadow-xl text-white transition-all transform active:scale-95 ${formData.isAuction ? 'bg-gradient-to-r from-purple-600 to-indigo-600 shadow-purple-200' : (listingType === 'affiliate' ? 'bg-gradient-to-r from-orange-500 to-red-500' : 'bg-primary hover:bg-primaryHover shadow-primary/20')}`}>
                {loading ? 'Đang xử lý...' : (isEditing ? 'Cập nhật tin' : (formData.isAuction ? 'Kích hoạt đấu giá' : (remainingPosts === 1 ? 'Đăng tin cuối cùng' : 'Đăng tin ngay')))}
            </button>
          </form>
